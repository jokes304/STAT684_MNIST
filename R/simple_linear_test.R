
set.seed = 1

if (! require('dslabs')) {
  install.packages('dslabs')
  require('dslabs')
}
if (! require('lattice')) {
  install.packages('lattice')
  require('lattice')
}
if (! require('neuralnet')) {
  install.packages('neuralnet')
  require('neuralnet')
}

if (! require('ggplot2')) {
  install.packages('ggplot2')
  require('ggplot2')
}

if (! require('tensorflow')) {
  install.packages('tensorflow')
  require('tensorflow')
}

if (! require('reshape2')) {
  install.packages('reshape2')
  require('reshape2')
}

if (! require('tfestimators')) {
  install.packages('tfestimators')
  require('tfestimators')
}

if (! require('reticulate')) {
  install.packages('reticulate')
  require('reticulate')
}

if (! require('mltools')) {
  install.packages('mltools')
  require('mltools')
}

if (! require('caret')) {
  install.packages('caret')
  require('caret')
}

path_to_python <- "C:/Users/Andrew/AppData/Local/r-miniconda/envs/tf/"    
use_python(path_to_python,required=TRUE)

library(tensorflow)

#install_tensorflow(version="2.0.0", envname = "tf")

mnist = read_mnist()
train_images<-mnist$train$images/255
dt_labels <- as.factor(mnist$train$labels)
dt_lab = as.data.frame (dt_labels)
sdfls<-dummyVars(" ~ .",data=dt_lab)

train_labels <- data.frame(predict(sdfls, newdata = dt_lab))

label_1<-train_labels[1,]
image_1<-train_images[1,]

lda_fit <- lda(train_images, train_labels)




mnist$train$images <- mnist$train$images/255
mnist$test$images <- mnist$train$images/255

#mnist$train$labels

use_condaenv("tf", required=TRUE)

classifier <- linear_classifier(
  feature_columns = feature_columns(
    column_numeric("x", shape = shape(784L))
  ),
  n_classes = 10L  # 10 digits
)

classifier <- linear_classifier(
  feature_columns = feature_columns(
    tf$feature_column$numeric_column("images", shape = shape(784L))
  ),
  n_classes = 10L  # 10 digits
)

save = tf$feature_column$numeric_column("images", shape = shape(784L))
ff = feature_columns(save)

classifier <- linear_classifier(
  feature_columns = ff,
  n_classes = 10L  # 10 digits
)
